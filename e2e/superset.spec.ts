...
test.describe('Superset Analytics', () => {
  ...
  test('Dashboard: "Book Store Analytics" opens and renders charts', async ({ page }) => {
    ...
    // Superset dashboards load charts asynchronously — give extra time for data queries
    await page.waitForTimeout(8_000)
    await page.screenshot({ path: 'screenshots/superset-09-bookstore-dashboard.png', fullPage: true })

    // Broad selector covering multiple Superset chart rendering modes
    const charts = page.locator('.chart-container, .dashboard-chart, canvas, svg.recharts-surface, .superset-legacy-chart, [data-test="chart-container"], .slice_container')
