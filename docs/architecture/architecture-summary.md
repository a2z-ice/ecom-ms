...
The cert-dashboard-operator is a Go-based Kubernetes operator (OLM-managed, Capability Level 3) that deploys a web dashboard for monitoring and renewing cert-manager certificates. It displays certificate lifecycle status with color-coded progress bars (green > 10d, yellow <= 10d, red <= 5d), provides one-click renewal via a confirmation dialog with token input (password-masked, clipboard copy icon for kubectl command, Show/Hide toggle) and SSE live streaming, and auto-refreshes every 30 seconds. Accessible at NodePort 32600. Security hardened with Kubernetes TokenReview authentication on mutating endpoints (auth checked before rate limit), rate limiting (1/10s), CRD validation webhook, and 5 Prometheus custom metrics (`cert_dashboard_*`) for operational observability. Pod security follows the Kubernetes restricted PSS profile (seccomp RuntimeDefault, capabilities drop ALL, non-root uid 1000).
